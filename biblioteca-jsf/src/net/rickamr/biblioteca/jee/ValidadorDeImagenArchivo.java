...
/** Valida los valores de tipo Archivo anotados con <code>@ArchivoImagen</code>.
 * Verifica que el archivo tenga un formato de imagen que pueda manejar Java.
 * Normalmente se usa en las clases entity. */
public class ValidadorDeImagenArchivo
        implements ConstraintValidator<Imagen, Archivo> {
   ...
   *         caso contrario. */
  @Override
  public boolean isValid(Archivo value, ConstraintValidatorContext context) {
    if (value == null) {
      return true;
    } else {
      try {
        return value.isImagen();
